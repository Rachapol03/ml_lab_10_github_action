...
def transition_model_alias(model_name: str, alias: str) -> None:
    """
    Set (or update) an alias for the latest version of a registered model.
    This uses MLflow's alias mechanism (modern replacement for stages).
    """
    client = MlflowClient()
    try:
        # Find latest version by max(version)
